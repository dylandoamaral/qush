...
// eslint-disable-next-line no-unused-vars
import { IOEither, rightIO, getIOValidation, map as mapIOEither, chain, fromEither } from "fp-ts/lib/IOEither";
// eslint-disable-next-line no-unused-vars
 ...
 * @param preset
 */
const validatePreset = (args: string[], preset: Preset): Either<NonEmptyArray<string>, void> => {
    /**
    ...
    const need = (sequence: string, sentence: string): Either<NonEmptyArray<string>, void> => {
        if (!uniq(sequence, sentence)) {
            if (sentence.includes(sequence)) return left([errorTemplateMultiple(sequence, sentence)]);
    ...
    const excess = (sequence: string, sentence: string): Either<NonEmptyArray<string>, void> => {
        if (sentence.includes(sequence)) return left([errorTemplateExcess(sequence, sentence)]);
        return right(null);
    };
     ...
     * @param map_name
     */
    const exist = (key: string, map: Map<string, Field>, map_name: string): Either<NonEmptyArray<string>, void> => {
